...
func (h *Handler) List(c fiber.Ctx) error {
	...
	isGlobal := query.QueryBoolPtr(c, "is_global")

	roles, err := h.service.List(c.Context(), page, limit, search, sortBy, sortDir, isGlobal)
	if err != nil {
...
func (h *Handler) ListByServiceID(c fiber.Ctx) error {
	...
	roles, err := h.service.ListByServiceID(c.Context(), serviceID, page, limit, search, sortBy, sortDir)
	if err != nil {
		return fiber.NewError(fiber.StatusInternalServerError, "Внутренняя ошибка")
	}
	return c.JSON(roles)
